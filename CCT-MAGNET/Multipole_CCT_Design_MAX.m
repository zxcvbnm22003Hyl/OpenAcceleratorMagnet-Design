...
LINEAR_ELLIPTIC_PARAMS.N_pole = 1;           % 磁场阶数 - 决定磁场的多极性
...
% --- 专业版TNB计算函数 (已修复：显式几何参数，移除不可靠的dbstack探测) ---
% geom 结构体字段说明：
%   geom.type : 'linear'       - 直线型（中心轴为Z轴）
%               'torus_xz'     - 标准圆环，环芯在XZ平面，绕Y轴弯曲（弯曲圆形/椭圆CCT）
%               'torus_xy'     - 标准圆环，环芯在XY平面，绕Z轴弯曲
%               'bent_poly_yz' - 弯曲拟多边形，在YZ平面弯曲，环心在(0,-R_bend,0)
%   geom.Rc    : 标准圆环半径（torus_xz / torus_xy 时使用）
%   geom.R_bend: 弯曲半径（bent_poly_yz 时使用）
function full_path_data = calculate_tnb_frames_pro(path_xyz, show_progress, geom)
    ...
    % 2. 根据显式几何类型计算参考法向 N_ref（始终由导体骨架中心指向外侧）
    N_ref = zeros(num_pts, 3);
    
    switch lower(geom.type)
        case 'linear'
            % === 直线型 === 中心轴为Z轴，N_ref指向XY平面外侧
            N_ref = [path_xyz(:, 1), path_xyz(:, 2), zeros(num_pts, 1)];
            
        case 'torus_xz'
            % === 标准圆环（绕Y轴弯曲）=== 环芯圆在XZ平面，半径Rc
            % 导管中心坐标 = (Rc*x/|xz|, 0, Rc*z/|xz|)，即XZ投影到半径Rc处
            Rc = geom.Rc;
            xz_norm = max(sqrt(path_xyz(:,1).^2 + path_xyz(:,3).^2), 1e-9);
            core_x = path_xyz(:,1) .* (Rc ./ xz_norm);
            core_z = path_xyz(:,3) .* (Rc ./ xz_norm);
            % N_ref = 导线位置 - 导管中心，即从管心指向导线的外法向
            N_ref = [path_xyz(:,1) - core_x, path_xyz(:,2), path_xyz(:,3) - core_z];
            
        case 'torus_xy'
            % === 标准圆环（绕Z轴弯曲）=== 环芯圆在XY平面，半径Rc
            Rc = geom.Rc;
            xy_norm = max(sqrt(path_xyz(:,1).^2 + path_xyz(:,2).^2), 1e-9);
        ...
        case 'bent_poly_yz'
            % === 弯曲拟多边形（YZ平面弯曲）===
            % 弯曲变换：Z_直线→弧长，Y_直线→管道径向，X_直线→横向（不变）
            % 环芯圆在YZ平面，圆心在 (0, -R_bend, 0)，半径 R_bend
            % 导管中心 = (0, R_bend*cos(α)-R_bend, R_bend*sin(α))
            % 其中 α = atan2(z, y+R_bend)，即 yz_norm = sqrt((y+R_bend)^2 + z^2)
            R_bend = geom.R_bend;
            yz_norm = max(sqrt((path_xyz(:,2) + R_bend).^2 + path_xyz(:,3).^2), 1e-9);
            % N_ref_X = x（横向分量，与弯曲无关，直接作为外法向分量）
            % N_ref_Y = (y+R_bend)*(1 - R_bend/yz_norm) = y - core_Y
            % N_ref_Z = z*(1 - R_bend/yz_norm) = z - core_Z
            N_ref = [path_xyz(:,1), ...
                     (path_xyz(:,2) + R_bend) .* (1 - R_bend ./ yz_norm), ...
                     path_xyz(:,3) .* (1 - R_bend ./ yz_norm)];
            
        otherwise
            % 未知类型，退回直线型
            warning('calculate_tnb_frames_pro: 未知的geom.type "%s"，使用线型默认值。', geom.type);
            N_ref = [path_xyz(:, 1), path_xyz(:, 2), zeros(num_pts, 1)];
    end

    % 3. 计算最终严格正交的法向 N 和副法向 B
...
function result = generate_linear_circular_cct()
    ...
    if tnb_method == 1 % 如果选择RMF算法
        full_data_1 = calculate_tnb_frames_pro(xyz1, output_params.show_progress, geom);
        full_data_2 = calculate_tnb_frames_pro(xyz2, output_params.show_progress, geom);
    else % 如果选择简化算法
...
function result = generate_curved_circular_cct()
    ...
    fprintf('📊 计算TNB标架...\n');
    % 根据弯曲轴方向选择正确的几何类型
    if strcmpi(params.bend_axis, 'Y')
        geom.type = 'torus_xz'; % 绕Y轴弯曲：环芯圈在XZ平面
    else
        geom.type = 'torus_xy'; % 绕Z轴弯曲：环芯圈在XY平面
    end
    geom.Rc = params.Rc;
    if tnb_method == 1 % 使用RMF算法
        full_data_1 = calculate_tnb_frames_pro(xyz1, output_params.show_progress, geom);
...
function result = generate_curved_elliptic_cct()
    ...
    % 计算TNB标架
    fprintf('📊 计算TNB标架...\n');
    geom.type = 'torus_xz'; % 弯曲椭圆始终绕Y轴弯曲：环芯圈在XZ平面
    geom.Rc = params.Rc;
    if tnb_method == 1 % 使用RMF算法
        full_data_1 = calculate_tnb_frames_pro(xyz1, output_params.show_progress, geom);
        full_data_2 = calculate_tnb_frames_pro(xyz2, output_params.show_progress, geom);
...
function result = generate_bent_quasipolygonal_cct()
    ...
    % 计算TNB标架
    fprintf('📊 计算最终TNB标架...\n');
    % 弯曲拟多边形：Z直线方向被弯曲到YZ平面弧，Y方向成径向，X方向不变
    % 环心圆在YZ平面，圆心在(0, -R_bend, 0)，半径R_bend
    geom.type = 'bent_poly_yz';
    geom.R_bend = param.R_bend;
    if tnb_method == 1 % 使用RMF算法
        full_data_1 = calculate_tnb_frames_pro(final_path_1, output_params.show_progress, geom);
        full_data_2 = calculate_tnb_frames_pro(final_path_2, output_params.show_progress, geom);
    else % 使用简化算法
